 ...
 */
import { ITEM_DEFINITIONS, ITEM_TRAIT_INDEX, type ItemRegistry, type ItemTraitIndex } from "$lib/domain/items";
import { applyInventoryEffect } from "$lib/domain/systems/item-effect-system";
import type { Inventory } from "$lib/domain/systems/inventory-system";
import {
  canIgnite,
  decayRateMultiplier,
...
} from "$lib/domain/exposure/exposure-context";
...
/**
 * Unified entry point for environmental effects. Processes all instantaneous
 * reactions (temperature, ignition) triggered by the current exposure context.
 * Time-based decay is handled separately via `tickDecay`.
 */
export function evaluateEnvironmentalExposure(
  inventory: Inventory,
  ctx: ExposureContext,
  ...
  const tempRes = processTemperatureReactions(next, ctx.ambientTemp, rng, index, defs);
  next = tempRes.inventory;
  reactions.push(...tempRes.reactions);

  // Flammability / Ignition
  const flamRes = processFlammableReactions(next, ctx, rng, index, defs);
  next = flamRes.inventory;
  reactions.push(...flamRes.reactions);
...
export function processTemperatureReactions(
  ...
  for (const itemId of index.temperatureSensitive) {
    ...
    if (temperature > trait.maxSafeTemp || temperature < trait.minSafeTemp) {
      const updated = applyInventoryEffect(next, itemId, trait.effect, rng);
      if (updated !== next) {
        next = updated;
