...
describe("tickPlacedItemExposure", () => {
  it("shows a warning for smoldering branch near fire on the ground", () => {
    ...
    const ctx = { location: "ground" as const, ambientTemp: 20, nearFire: true };
  ...
  it("does not ignite a branch if it is sealed in a container near fire", () => {
    ...
    const ctx = { location: "sealed" as const, ambientTemp: 20, nearFire: true };
  ...
  it("decays berries on the ground faster near fire", () => {
    ...
    const ctx = { location: "ground" as const, ambientTemp: 20, nearFire: true }; // nearFire decay rate multiplier * 2
