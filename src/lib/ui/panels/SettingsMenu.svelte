...
/**
 * SettingsMenu.svelte
 * Unified Settings panel for Ashenmoon.
 * Provides tabs:
 * 1. Controls (Keybinds configuration & UI Preferences)
 * 2. Audio (Volume sliders with categories)
 */
...
// Component states
let activeTab = $state<"controls" | "audio">("controls");
...
const VOLUME_ROWS: { key: VolumeKey; label: string }[] = [
  ...
  { key: "entities", label: "Entities Volume" },
];
...
/** Handles slider value changes - plays a small audio preview when appropriate. */
function handleVolumeChange(key: VolumeKey, value: number) {
  setVolume(key, value);
  // Play a quiet UI inventory click as volume feedback
  if (key === "ui" || key === "master" || key === "sfx") {
    playSound("ui.inventory.click");
  }
}
</script>
  ...
  >
    ...
    <div class="modal-body">
      ...
      {:else if activeTab === "audio"}
        <p class="description">
          Adjust game sound and category volumes.
        </p>

        <div class="audio-preferences-section">
          <label class="preference-row">
            <div class="preference-info">
              <span class="preference-name">Mute All Audio</span>
              <span class="preference-desc">Silence all game audio and effects</span>
            </div>
            <input
              type="checkbox"
          ...
          {#each VOLUME_ROWS as row (row.key)}
            <label class="preference-row volume-row">
              ...
              <input
                ...
                onchange={(e) => handleVolumeChange(row.key, Number(e.currentTarget.value))}
              />
            </label>
          {/each}
        </div>
...
<style>
  ...
  .tab-btn.active {
    color: rgba(255, 220, 120, 0.95);
    border-bottom-color: rgba(255, 220, 120, 0.8);
  ...
  .volume-slider {
    width: 140px;
    accent-color: rgba(255, 220, 120, 0.8);
    cursor: pointer;
  }
