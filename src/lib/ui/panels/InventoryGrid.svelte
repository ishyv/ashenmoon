...
import { getItemDef, iconUrlFor, reactsInto, traitOf } from "$lib/domain/items";
...
function getStashUsage(): number {
  if (!rpgState.inventory) return 0;
  return Object.values(rpgState.inventory.slots).reduce((sum: number, slot: any) => {
