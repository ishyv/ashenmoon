...
const navItems = $derived(
  [
    ...
    { label: "scripts", href: `/guilds/${guildId}/scripts` },
  ].map((item) => ({
    ...item,
...
<style>
  ...
  .brand :global(.hyvui-breadcrumb) { min-width: 0; }
  .brand :global(.hyvui-breadcrumb-current) {
    min-width: 0;
    overflow: hidden;
  ...
  @media (max-width: 48rem) {
    :global(.guild-topbar .hyvui-topbar-inner) {
    ...
    :global(.guild-topbar .hyvui-topbar-center:empty) { display: none; }
    :global(.guild-topbar .hyvui-topbar-right) { justify-content: flex-start; }

    .body { grid-template-columns: 1fr; }
    .brand { width: 100%; gap: var(--space-xs); }
    .topbar-actions {
      width: 100%;
